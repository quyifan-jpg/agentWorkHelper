...
export class WebSocketClient {
  ...
  private messageQueue: string[] = []; // 消息队列，用于断线重连后发送
      ...
      try {
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            // console.log(
            //   `[WebSocket底层] 收到消息，当前有${this.messageHandlers.length}个处理器`
            // );
  ...
  onMessage(handler: (message: WsMessage) => void): void {
    // console.log(
    //   `[WebSocket] 添加消息处理器，当前共${this.messageHandlers.length + 1}个`
    // );
      ...
      this.reconnectTimer = setTimeout(() => {
        this.connect()
          .then(() => {
            ...
            // console.log(
            //   `[WebSocket] 重连成功，重新注册了${this.messageHandlers.length}个处理器`
            // );
          })
          .catch((error) => {
